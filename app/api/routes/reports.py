...
async def list_reports(
    ...
    dao = ReportDao()
    reports, total = dao.list_by_user(current_user.user_id, report_type=report_type, page=page, page_size=page_size)
    return {
        "data": reports,
...
async def get_report(report_id: int, current_user: TokenData = Depends(get_current_user)):
    """Get report detail."""
    dao = ReportDao()
    report = dao.get_by_id(report_id, current_user.user_id)
    if not report:
