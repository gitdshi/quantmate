...
datasync_api_calls_total = Counter(
    ...
    'Total number of Tushare API calls',
    ['api_name']
)

datasync_api_errors_total = Counter(
    'datasync_api_errors_total',
    'Total number of Tushare API errors',
    ['api_name']
)
...
datasync_rows_ingested_total = Counter(
    ...
    'Total number of rows successfully ingested into database tables',
    ['table']
)

# Failed steps counter (by step/ingest function name)
datasync_failed_steps_total = Counter(
    'datasync_failed_steps_total',
    'Total number of failed ingestion steps',
    ['step']
)
...
datasync_backfill_lock_status = Gauge(
    ...
    'Status of datasync backfill lock (0 = cannot acquire, 1 = healthy)'
)
...
def metrics_hook(api_name: str, success: bool, duration: float, rows: int, error: Optional[str] = None):
    ...
    if not success:
        ...
        if error and any(token in error.lower() for token in ['rate limit', 'too many requests', '每分钟最多访问', '接口访问太频繁', '后重试', '频率']):
