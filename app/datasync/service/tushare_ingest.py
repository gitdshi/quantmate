    ...
    for i in range(0, total, batch_size):
        ...
        for ts_code in chunk:
            ...
            try:
                ...
                df = call_pro(
                    "dividend", ts_code=ts_code, start_date=s_norm.replace("-", ""), end_date=e_norm.replace("-", "")
                )
                ...
                for r in df.to_dict(orient="records"):
                    ...
                    if not ann_dt:
                        ...
                        logging.debug(
                            "Skipping dividend row without ann_date and imp_ann_date for %s", r.get("ts_code")
                        )
                        continue
                    ...
                    rows_to_insert.append(
                        {
                            ...
                            "ann_date": ann_dt,
                            "imp_ann_date": (
                                pd.to_datetime(r.get("imp_ann_date")).date() if r.get("imp_ann_date") else None
                            ),
                            "record_date": (
                                pd.to_datetime(r.get("record_date")).date() if r.get("record_date") else None
                            ),
        ...
        for ts_code in chunk:
            ...
            try:
                if progress_cb:
                    progress_cb(ts_code=ts_code, cursor_date=e_norm)
                df = call_pro(
                    "adj_factor", ts_code=ts_code, start_date=s_norm.replace("-", ""), end_date=e_norm.replace("-", "")
                )
                if df is None or df.empty:
                    continue
                rows_to_insert = []
