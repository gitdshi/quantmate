...
def get_last_sync_date(symbol: str, exchange: str, interval: str = "d") -> Optional[date]:
    with engine.connect() as conn:
        res = conn.execute(
            text(
                "SELECT last_sync_date FROM sync_status WHERE symbol=:symbol AND exchange=:exchange AND `interval`=:interval"
            ),
...
def get_bar_stats(
    symbol: str, exchange: str, interval: str = "d"
) -> Tuple[int, Optional[datetime], Optional[datetime]]:
    with engine.connect() as conn:
        res = conn.execute(
            text(
                "SELECT COUNT(*), MIN(`datetime`), MAX(`datetime`) FROM dbbardata WHERE symbol=:symbol AND exchange=:exchange AND `interval`=:interval"
            ),
            {"symbol": symbol, "exchange": exchange, "interval": interval},
        )
        row = res.fetchone()
...
def upsert_dbbaroverview(
    symbol: str, exchange: str, interval: str, count: int, start: Optional[datetime], end: Optional[datetime]
):
    with engine.begin() as conn:
        conn.execute(
            text(
