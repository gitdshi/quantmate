...
class DataSourceItemDao:
    ...
    def list_all(self, source: Optional[str] = None) -> list[dict[str, Any]]:
        with connection("quantmate") as conn:
            if source:
                ...
                rows = conn.execute(text("SELECT * FROM data_source_items ORDER BY source, id")).fetchall()
    ...
    def list_enabled(self, source: Optional[str] = None) -> list[dict[str, Any]]:
        with connection("quantmate") as conn:
            if source:
                rows = conn.execute(
                    text("SELECT * FROM data_source_items WHERE enabled = 1 AND source = :src ORDER BY id"),
                    {"src": source},
                ).fetchall()
            else:
                rows = conn.execute(
