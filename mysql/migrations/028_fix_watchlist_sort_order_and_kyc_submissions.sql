...
-- Guard: add description column if missing (may be absent on older staging DBs)
SET @has_watchlist_description := (
    SELECT COUNT(*)
    FROM information_schema.columns
    WHERE table_schema = DATABASE() AND table_name = 'watchlists' AND column_name = 'description'
);
SET @add_watchlist_description_sql := IF(
    @has_watchlist_description = 0,
    'ALTER TABLE `quantmate`.`watchlists` ADD COLUMN `description` VARCHAR(500) DEFAULT NULL AFTER `name`',
    'SELECT 1'
);
PREPARE stmt_add_watchlist_description FROM @add_watchlist_description_sql;
EXECUTE stmt_add_watchlist_description;
DEALLOCATE PREPARE stmt_add_watchlist_description;
