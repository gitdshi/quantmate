-- Recreate MFA/API-key/session tables lost from the staging database.
--
-- Migration 009 created these with CREATE TABLE IF NOT EXISTS, but the staging
-- database lost all three tables after a manual restore (same class of drift
-- migration 062 fixed for alerts tables), and 009 is already recorded as
-- applied so it will never re-run. API key creation fails with
-- "Table 'quantmate.api_keys' doesn't exist" until these are restored.
-- Idempotent: safe to run on environments that still have the tables.
