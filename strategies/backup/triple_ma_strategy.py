...
class TripleMAStrategy(CtaTemplate):
    ...
    parameters = [
        "fast_window", "mid_window", "slow_window", "fixed_size",
        "stop_loss_window", "fixed_stop_multiplier", "trailing_stop_multiplier", "use_stop_loss"
    ]
    
    variables = [
        "fast_ma", "mid_ma", "slow_ma", "ma_trend",
        "entry_price", "fixed_stop", "trailing_stop"
    ]
    ...
    def on_bar(self, bar: BarData):
        ...
        if self.pos != 0 and self.use_stop_loss:
            ...
            if state:
                ...
                if self.stop_loss_manager.should_stop_loss(vt_symbol, bar.close_price):
                    ...
                    if self.pos > 0:
                        self.write_log(f"多头止损触发 ({reason}): 当前价={bar.close_price:.2f}, 止损价={active_stop:.2f}")
                        self.sell(bar.close_price * 0.99, abs(self.pos))
                    elif self.pos < 0:
                        self.write_log(f"空头止损触发 ({reason}): 当前价={bar.close_price:.2f}, 止损价={active_stop:.2f}")
        ...
        if self.pos == 0:
            ...
            if self.ma_trend == 1:
                ...
                self.write_log(f"多头开仓信号: 快线={self.fast_ma:.2f} > 中线={self.mid_ma:.2f} > 慢线={self.slow_ma:.2f}")
        ...
        # 持有多头时
            ...
            if self.fast_ma < self.mid_ma:
                self.sell(bar.close_price * 0.99, abs(self.pos))
                self.write_log(f"多头平仓信号: 快线下穿中线")
    ...
    def on_trade(self, trade: TradeData):
        ...
        if self.use_stop_loss and len(recent_closes) >= 2:
            if trade.direction == Direction.LONG:
                ...
                self.entry_price = trade.price
                state = self.stop_loss_manager.set_entry(
                    vt_symbol, trade.price, recent_closes, is_long=False
                )
                self.fixed_stop = state.fixed_stop_price
                self.trailing_stop = state.trailing_stop_price
