...
class StopLossManager:
    ...
    def set_entry(
        self, vt_symbol: str, entry_price: float, recent_prices: List[float], is_long: bool = True
    ) -> StopLossState:
        """
        开仓时设置止损价格
    ...
    def update_trailing_stop(self, vt_symbol: str, current_price: float, recent_prices: List[float]) -> Optional[float]:
        """
        更新移动止损价格
...
def calculate_fixed_stop_loss(
    entry_price: float, recent_prices: List[float], std_multiplier: float = 2.0, is_long: bool = True
) -> float:
    """
    计算固定止损价格
